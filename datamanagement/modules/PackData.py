...
class PackData(LModule):
    def __init__(self,userName = None):
        ...
        options.add('output', 'o', 'Output folder', mandatory=False, helpmessage=' (in each node) where the packed data is stored. If not provided, the same parent path of the input MS is used')
        options.add('mode', 'm', 'Tar mode', default = 'cvf')
        ...
        # the information
        information = 'Pack (TAR) the data in a RefFile.'
    ...
    def function(self, identifier, what, childIndex, grandChildIndex, taskIndex):
        ...
        command = "cd " + outputPath + " ; tar " + self.mode + " " + absPath + ".tar" + " " + msoperations.getMeasurementSetName(absPath)
        if self.delete:
