...
def functionForSSH(absPath, node):
    size = msoperations.addImagingColumns(absPath, node)
    if type(size) == str:
        # Error happened
        error = size
        print error
...
class AddImagingColumns(LModule):
    def __init__(self,userName = None):
        ...
        # the information
        information = 'Add imaging columns to the MSP pointed by RefFile'
    ...
    def function(self, identifier, what, childIndex, grandChildIndex, taskIndex):
        ...
        # Define commands
        commandAdd = 'source ' + self.initfile + ' ; python -c "import ' + __name__ + '; ' + __name__ + '.' + functionForSSH.__name__ + '(\\\"' + absPath +'\\\", \\\"' + node +'\\\")"'
        if node != currentNode:
    ...
    def process(self, reffile, numprocessors, numnodes, logspath,query,initfile):
        self.query = query
