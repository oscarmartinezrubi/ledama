...
def functionForSSH(absPath):
    os.system("taql 'update " + absPath + " set FLAG=False, FLAG_ROW=False'")

class UnFlagData(LModule):
    def __init__(self,userName = None):
        ...
        # the information
        information = 'Unflag to data pointed by a RefFile'
    ...
    def function(self, identifier, what, childIndex, grandChildIndex, taskIndex):
        ...
        commandUnflag = 'source ' + self.initfile + ' ; python -c "import ' + __name__ + '; ' + __name__ + '.' + functionForSSH.__name__ +'(\\\"' + absPath +'\\\")"'
        if node != currentNode:
            commandUnflag =  "ssh " + node + " '" + commandUnflag + "'"
        unflagerr = td.execute(commandUnflag)[1]
        if unflagerr != '':
