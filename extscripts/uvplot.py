...
Written by George Heald, modified by Oscar Martinez and Ger van Diepen
...
version 1.28 01/03/2012: Possible to select phaserate as y axis
version 1.29 01/11/2012: Added option to scale the shown statistics (default 1000)
version 1.30 03/12/2012: Use mscal.ha() instead of adding derived columns
                         remove duplicate sorting in first and last time acquisition
version 1.31 18/12/2012: Use table.calc instead of table.get
...
version_string = 'v1.31, 18 December 2012\nWritten by George Heald, modified by Oscar Martinez and Ger van Diepen'
...
def main(options):
    ...
    scaleStatsFactor = int(options.statisticsscale)
    ...
    if showStats:
        if scaleStatsFactor > 1:
            print 'WARNING: Before computing statistics the values are multiplied by ' + str(scaleStatsFactor)
    ...
    axisLabels = {'time': 'Time',
                  ...
                  'phase_rate': 'Visibility phase rate [radians]'}
    ...
    for tpart in tsel.iter(["ANTENNA1","ANTENNA2"]):
        ...
        if operation != 0:
            ...
            addInfo(showStats, scaleStatsFactor, tmpvals[~tmpvals.mask], label, labXPositions[1], labYPositions[1])
        else:
            for j in polarizations:
                ...
                addInfo(showStats, scaleStatsFactor, tmpvals[~tmpvals.mask], polLabels[j], labXPositions[j], labYPositions[j])
        ppgplot.pgpage()
...
def addInfo(showStats, scaleStatsFactor, values, label, xPosition, yPosition):
    if showStats:
        if scaleStatsFactor >1:
            nvalues = values * float(scaleStatsFactor)    
        else:
            nvalues = values
    ...
    if operation == 0:
        if axisname == 'amp':
            ...
            if tmp != None:
                ...
                if axisname == 'phase':
                    if unwrap:
                        return numpy.ma.array(numpy.unwrap(tmp3,axis=0), mask=tmp3.mask)
                    else:
                        return tmp3
                else:
                    #phase_rate
                    # we need to wrap it first
                    tmp4=numpy.diff(numpy.unwrap(tmp3,axis=0),axis=0)
                    return numpy.ma.array(numpy.append(tmp4,numpy.array([tmp4[-1],]),axis=0), mask=tmp3.mask)
            return None                    
        else:
            print 'Error: Requested axis not implemented'
            return None
        ...
        if axisname == 'real':
            ...
            phaseop = numpy.ma.arctan2(imagop,realop)
            ...
            if axisname == 'phase':
                ...
                #phase_rate
                unwrappedarray = numpy.ma.transpose(phaseop)
                tmp=numpy.diff(numpy.unwrap(unwrappedarray))
                return numpy.ma.array(numpy.append(tmp,numpy.array([tmp[-1],])), mask=unwrappedarray.mask)
        else:
            print 'Error: Requested axis not implemented'
            return None    
    elif operation == 2:
...
opt.add_option('-y','--yaxis',help='Y axis type (choose from amp|phase|real|imag|phaserate) [default amp]',default='amp',type='choice',choices=['amp','phase','real','imag','phaserate'])
...
opt.add_option('-c','--column',help='Column to plot. The options are DATA|CORRECTED_DATA|MODEL_DATA but also other columns that the user may have created. It is actually also possible to specify any expression supported by TaQL, e.g. DATA-CORRECTED_DATA. [default DATA]',default='DATA')
...
opt.add_option('-l','--statisticsscale',help='Statistics scale factor [default 1000]. Values are multiplied by this number before the statistics are computed ',default=1000)
...
class GuiFrontend:
    ...
    def __init__(self, master, options):
        ...
        self.yaxisDropbox = OptionMenu(self.frame, self.yvar, "amp", "phase","real","imag", "phaserate")
        ...
        self.scaleStatsLabel = Label(self.frame, text="Statistics scale factor")
        self.scaleStatsEntry = Entry(self.frame)
        self.scaleStatsEntry.insert(0, self.myoptions.statisticsscale)
        ...
        guirow += 1
        self.statsCheck.grid(row=guirow,column=0,sticky=W)
        self.scaleStatsLabel.grid(row=guirow,column=1,sticky=W)
        self.scaleStatsEntry.grid(row=guirow,column=2,sticky=W)
        guirow += 1
    ...
    def returnoptions(self):
        ...
        self.myoptions.scalestatistics = bool(self.scaleStatsEntry.get())
