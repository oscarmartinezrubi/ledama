...
class GetQFSs(LModule):
    def __init__(self, userName = None):
        self.userName = userName
        if self.userName == None:
            self.userName = utils.getUserName()
        options = LModuleOptions()
        options.add('qfsids','q','QFS IDs', mandatory=False)
...
We can use this LModule to obtain DiagFiles (with data or with only the query) to be used in the plotting LModules"""
    ...
    def process(self, qfsids, ldss, ldsbids, ldsbpids,  msids, subbands, sbcf, qkindids, full, output, timeout, dbname, dbuser, dbhost):
        qm = QueryManager()
        queryOption = QFS_KEY
        names = None
        if output != '':
        ...
        if qfsids != "":
            qm.addCondition(queryOption, qc, QFSTAT+ID, tuple(utils.getElements(qfsids)))
        if ldss != "":
