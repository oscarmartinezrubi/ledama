...
class GetQTSs(LModule):
    def __init__(self, userName = None):
        self.userName = userName
        if self.userName == None:
            self.userName = utils.getUserName()
        options = LModuleOptions()
        options.add('qtsids','q','QTS IDs', mandatory=False)
...
We can use this LModule to obtain DiagFiles (with data or with only the query) to be used in the plotting LModules"""
    ...
    def process(self, qtsids, ldss, ldsbids, ldsbpids,  msids, subbands, sbcf, qkindids, full, output, timeout, dbname, dbuser, dbhost):
        qm = QueryManager()
        queryOption = QTS_KEY
        names = None
        if output != '':
        ...
        if qtsids != "":
            qm.addCondition(queryOption, qc, QTSTAT+ID, tuple(utils.getElements(qtsids)))
        if ldss != "":
